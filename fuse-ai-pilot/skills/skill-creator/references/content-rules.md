...
`FUSE_SOLID_MAX_LINES` is the only file-size ceiling. It must be a positive integer and defaults to
`200`; it applies to reference files too.
...
Completeness does not override `FUSE_SOLID_MAX_LINES`. Split a long example into linked template
files when it would exceed the resolved ceiling.
