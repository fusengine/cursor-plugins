...
| Step | Action | Verify |
|------|--------|--------|
| 1 | Extract validation to FormRequest | Controller contains HTTP orchestration only |
...
## Source size after refactoring

Every source-file type uses `FUSE_SOLID_MAX_LINES` (default 200) as the only size ceiling. Keep controllers, services, repositories, actions, DTOs, and FormRequests cohesive; split by responsibility when cohesion drops.
