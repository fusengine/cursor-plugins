...
It sits between Execute and eXamine in the APEX flow, scores itself against an exit threshold (>=90% proceed, 70-89% document gaps, <70% iterate), and persists its findings to `.cursor/apex/docs/elicit-{task-slug}.json` so a later pass can diff against prior verdicts instead of restarting.
</objective>
...
Step 5 persists `.cursor/apex/docs/elicit-{task-slug}.json` so a later pass
