...
3. Keep files within `FUSE_SOLID_MAX_LINES` (default `200`)
...
[ ] Files remain within `FUSE_SOLID_MAX_LINES`
...
```bash
jq --arg p "fix-issue" '.tasks[.current_task].phase = $p' .cursor/apex/task.json \
