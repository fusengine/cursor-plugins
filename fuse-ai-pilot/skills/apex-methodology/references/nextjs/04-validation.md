...
if [ ! -f ".cursor/apex/docs/elicit-${TASK_SLUG}.json" ]; then
  echo "❌ Missing .cursor/apex/docs/elicit-${TASK_SLUG}.json — go back to references/nextjs/03.5-elicit.md first."
  exit 1
fi
...
```text
If a file approaches `FUSE_SOLID_MAX_LINES` or accumulates multiple responsibilities:
main.tsx           # Core logic
...
### File Checks
- All files within `FUSE_SOLID_MAX_LINES`: PASS
