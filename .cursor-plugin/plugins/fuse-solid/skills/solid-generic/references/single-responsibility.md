...
description: SRP Guide - When and how to split files by responsibility, apply the repository source-size rule, and use modular paths for generic TypeScript
...
All source-file types use `FUSE_SOLID_MAX_LINES` (default 200) as the only size ceiling. Split by responsibility when cohesion drops rather than applying per-type numeric budgets.
...
-> `templates/module.md` - Cohesive module template
-> `templates/service.md` - Cohesive service template
