...
Check if `.cursor-plugin/marketplace.json` exists in the repo root.
...
## Marketplace Path (repo with .cursor-plugin/marketplace.json)
...
Keep only top-level directories that contain `.cursor-plugin/plugin.json`. If no plugins were modified, skip to Step M3 (still bump suite version).

### Step M2: Bump Plugin Versions

For each modified plugin: read `{name}/.cursor-plugin/plugin.json`, increment PATCH (`X.Y.Z` → `X.Y.(Z+1)`), write it back. Load `references/plugin-version-bump.md` for the marketplace entry synchronization rule.

### Step M3: Bump Suite Version + Recompute README Badges

Read `metadata.version` from `.cursor-plugin/marketplace.json`, increment PATCH (`X.Y.Z` → `X.Y.(Z+1)`), write it back.
...
For any plugin **added** in this commit, create its docs page and link it. Match the existing naming in `docs/plugins/` rather than assuming it from the raw top-level directory. Load `references/plugin-docs-parity.md` for the detection command and the full per-plugin checklist.
...
```bash
git add CHANGELOG.md README.md .cursor-plugin/marketplace.json */.cursor-plugin/plugin.json docs/
```
