...
2. If **absent**, ask the user **before any other work**:
   > "Activate the fuse-seo hook on this project? It is intended to validate meta/schema/OG after every matching Cursor Write operation on HTML-like files (.html, .astro, .tsx, .vue, .blade.php) through the @fusengine/harness seo scope; actual end-to-end Cursor execution and feedback remain runtime-unverified."
