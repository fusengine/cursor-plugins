...
1. **fuse-ai-pilot:explore-codebase** - Analyze existing PHP structure (`composer.json`, PSR-4 autoload map, `src/` layout, PHP version constraint, absence of `artisan`)
2. **fuse-ai-pilot:research-expert** - Verify latest PHP, PER Coding Style, PHPStan, and PHPUnit/Pest docs via Context7/Exa
...
**Read the `fuse-laravel:solid-php` skill before ANY code** — it already covers PHP-generic SOLID (`FUSE_SOLID_MAX_LINES`, default 200, as the source-size ceiling; interfaces separated; PHPDoc mandatory) beyond its Laravel auto-detection. Do NOT duplicate SOLID guidance locally (DRY).
