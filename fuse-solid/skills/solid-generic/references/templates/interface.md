...
## Claude Compatibility Hook System Interface

This nested `hookSpecificOutput` shape is the Claude Code compatibility response supported by
Cursor. Native Cursor hooks use event-specific top-level response fields instead; for
`preToolUse`, the documented fields are `permission`, `user_message`, `agent_message`, and
`updated_input`. Do not reuse one event's response contract for another event.
...
/** Claude Code compatibility hook output response. */
