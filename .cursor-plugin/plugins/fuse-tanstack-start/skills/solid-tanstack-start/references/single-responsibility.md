...
description: SRP for TanStack Start - repository source-size rule and responsibility-based splitting for routes, loaders, and server functions
...
All source-file types use `FUSE_SOLID_MAX_LINES` (default 200) as the only size ceiling. Split routes, server functions, server-only helpers, hooks, and services by responsibility when cohesion drops.
