...
```text
□ Focused views within `FUSE_SOLID_MAX_LINES`
...
| ARCH-04 | File Size | Within `FUSE_SOLID_MAX_LINES` (default `200`) |
...
```bash
jq --arg p "elicit" '.tasks[.current_task].phase = $p' .cursor/apex/task.json \
