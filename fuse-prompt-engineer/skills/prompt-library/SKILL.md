...
model: grok-4.6            # grok-4.6[effort=high] for a control organ
color: color-name
...
| `model` | Model to use | `grok-4.6` (every agent; `medium` is its default effort, so the value stays bare), `grok-4.6[effort=high]` (control organs only) |
