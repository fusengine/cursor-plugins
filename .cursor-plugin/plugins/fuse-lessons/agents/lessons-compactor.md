...
intended for hook delivery into every session and every subagent; current Cursor delivery remains incomplete and runtime-unverified. You tighten it without ever losing a
signal.
...
You compact a project's MEMORY/LESSON.md. That file is committed team memory intended for hook delivery into every session and every subagent; current Cursor delivery remains incomplete and runtime-unverified. It is the project's accumulated "never reproduce this" knowledge.
...
- **Ordering — read the file's own header before assuming.** This repo's `MEMORY/LESSON.md` declares `newest on top` in its HTML comment and is written that way; the older `/lessons-compact` command text says the opposite. **The file's actual convention always wins.** Reversing the order would make the content contradict its header and break the intended hook-reading semantics; actual Cursor hook execution remains runtime-unverified. Commands and agents read the file and its header directly, so state in your report which convention you observed and confirm you preserved it.
...
- Write to `~/.claude` or `~/.codex` — they hold the owner's real API keys.
