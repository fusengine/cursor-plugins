...
if [ ! -f ".cursor/apex/docs/elicit-${TASK_SLUG}.json" ]; then
  echo "❌ Missing .cursor/apex/docs/elicit-${TASK_SLUG}.json — go back to references/laravel/03.5-elicit.md first."
  exit 1
fi
...
find app -name "*.php" -exec wc -l {} + | awk -v max="$fuse_solid_max_lines" '$1 > max && $2 != "total"'
```
...
### File Checks
- Files exceeding `FUSE_SOLID_MAX_LINES`: 0
