...
Also defines the canonical output format of `design-system.md` — the file the local harness policy is intended to gate on — including its 4 hard requirements (the `## Design Reference` heading, a real inspiration URL, at least one chroma-positive OKLCH token, and the hard-forbidden font exclusions); actual Cursor enforcement is runtime-unverified.
...
<!-- Grounding: grounding-corpus.md §B (thresholds), §H (kept anchors); harness-contract.md invariant #2 (design-system.md format) + #3 (this file's read-path is intended to trigger harness phase 1; actual Cursor execution is runtime-unverified). -->
...
target skill — that read is intended to advance the local harness past phase 0; actual
Cursor state advancement is runtime-unverified. If a design
...
This is the generated file the local harness source is designed to gate and validate when
invoked (`gates-pipeline.ts` triggers on any read path ending `design-system.md`; `gates.ts`
validates its contents). Actual Cursor gate execution, blocking, and surfaced feedback are
runtime-unverified. Every requirement below is a **present/absent check**, not a suggestion —
missing one is intended to fail the gate silently downstream, with no error surfaced at write
time.
