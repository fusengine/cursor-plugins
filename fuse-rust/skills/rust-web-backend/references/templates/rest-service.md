...
A single-file service showing the standard stack wired together. Keep source files within `FUSE_SOLID_MAX_LINES` (default 200), and split into cohesive modules such as `routes/`, `error.rs`, `state.rs`, and `db.rs` when responsibilities diverge.
