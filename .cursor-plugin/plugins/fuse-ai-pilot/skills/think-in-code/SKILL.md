...
### 1. File-size audit (`FUSE_SOLID_MAX_LINES`, default `200`)
...
find . -type f \( -name "*.ts" -o -name "*.tsx" -o -name "*.py" -o -name "*.php" \) \
  ...
  | awk -v max="$fuse_solid_max_lines" '$1 > max && $2 != "total" {print $1, $2}' \
  | sort -rn | head -20
```
...
**Task:** "Find files exceeding `FUSE_SOLID_MAX_LINES` in `src/`."

- Before: 10 × Read full file → ~60KB tokens consumed, then mental wc.
- After: 1 × resolve `FUSE_SOLID_MAX_LINES`, then `find src -name '*.ts' | xargs wc -l | awk -v max="$fuse_solid_max_lines" '$1>max'` → ~1KB result.
