...
`FUSE_SOLID_MAX_LINES` (default 200) is the only source-file size ceiling. Split code when responsibilities diverge or cohesion drops, not at a separate file-type threshold.

| File | Purpose | Responsibility boundary |
|------|---------|-------------------------|
| `prisma.ts` | Singleton client | Client construction and lifecycle only |
| `schema.prisma` | Models, relations | Group models by a cohesive domain boundary |
| `seed.ts` | Database seeding | Orchestration only; extract reusable factories and scenarios |
