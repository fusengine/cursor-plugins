...
name: handler
applies-to: "**/*.go"
description: HTTP handler template - net/http or chi/echo, copy-paste ready, using the shared source-size ceiling
...
### Source-size ceiling
```go
// Keep handlers within FUSE_SOLID_MAX_LINES (default 200) and delegate to services.
```
...
- [ ] Handler stays within `FUSE_SOLID_MAX_LINES` (default 200) and delegates to a service
