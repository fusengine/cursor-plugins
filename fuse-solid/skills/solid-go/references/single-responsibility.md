...
- Stay within `FUSE_SOLID_MAX_LINES` (default 200) and delegate business logic
...
- Stay within `FUSE_SOLID_MAX_LINES` (default 200) and keep business logic cohesive
...
- Stay within `FUSE_SOLID_MAX_LINES` (default 200) and keep data access cohesive

### Model Layer
- Domain structs (User, Product, Order)
- Value objects
- Domain methods (User.IsActive())
- Stay within `FUSE_SOLID_MAX_LINES` (default 200) and keep domain data cohesive

---

## Shared Code (DRY)

If multiple features genuinely need shared logic:
...
- [ ] Each file has one responsibility and stays within `FUSE_SOLID_MAX_LINES` (default 200)
