...
If multiple features genuinely need the same code, move it to `modules/cores/`.
...
All source files use `FUSE_SOLID_MAX_LINES` (default 200) as the only size ceiling. Keep modules, services, validators, utilities, errors, constants, and entry points cohesive; split by responsibility when cohesion drops.
...
| Source files exceeding `FUSE_SOLID_MAX_LINES` (default 200) | Maintenance cost | -> Split by responsibility |
