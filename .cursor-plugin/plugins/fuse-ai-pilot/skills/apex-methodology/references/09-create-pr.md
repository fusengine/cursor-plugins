...
```markdown
## Summary
<!-- Concise bullets describing what this PR does -->
...
```text
✅ Cohesive, reviewable PRs
   - Easier to review
   - Faster feedback
   - Lower risk

⚠️ Broad but cohesive PRs
   - Split when responsibilities can be isolated safely
...
At the **start** of this phase, record it (and mark the task `completed` once the PR is opened) in `.cursor/apex/task.json`:

```bash
jq --arg p "create-pr" '.tasks[.current_task].phase = $p' .cursor/apex/task.json \
  > .cursor/apex/task.json.tmp && mv .cursor/apex/task.json.tmp .cursor/apex/task.json
# after the PR is created:
jq '.tasks[.current_task].status = "completed"' .cursor/apex/task.json \
  > .cursor/apex/task.json.tmp && mv .cursor/apex/task.json.tmp .cursor/apex/task.json
```
