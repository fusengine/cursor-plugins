...
<objective>
Enforces SOLID and DRY architecture on React 19 projects: source files stay within `FUSE_SOLID_MAX_LINES` (default 200) and split by responsibility when cohesion drops, types are isolated in `modules/[feature]/src/interfaces/` (never in component files), JSDoc is mandatory on exports, class components are forbidden, and existing logic/utilities must be grepped before writing new code (with `jscpd` duplication checks).
...
- Split components, hooks, and services by responsibility when cohesion drops.
...
| Template | Usage |
|----------|-------|
| `component.md` | React functional component |
| `hook.md` | Custom hook with TanStack Query |
