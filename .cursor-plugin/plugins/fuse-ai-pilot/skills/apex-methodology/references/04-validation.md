...
```bash
TASK_SLUG=$(jq -r '.current_task' .cursor/apex/task.json)
if [ ! -f ".cursor/apex/docs/elicit-${TASK_SLUG}.json" ]; then
  echo "❌ Missing .cursor/apex/docs/elicit-${TASK_SLUG}.json — go back to references/03.5-elicit.md first."
  exit 1
fi
if [ ! -f ".cursor/apex/docs/verify-${TASK_SLUG}.md" ]; then
  echo "❌ Missing .cursor/apex/docs/verify-${TASK_SLUG}.md — go back to the verification skill (runs between eLicit and eXamine) first."
  exit 1
fi
if [ ! -f ".cursor/apex/docs/challenge-${TASK_SLUG}.md" ]; then
  echo "❌ Missing .cursor/apex/docs/challenge-${TASK_SLUG}.md — the challenger runs SYSTEMATICALLY at every eLicit round and every Verify gate (Step 4.5 of 03.5-elicit.md, Step 6 of the verification skill), no exceptions. Go run it first."
  exit 1
fi
```
...
```text
□ All files within `FUSE_SOLID_MAX_LINES` (default `200`)
...
### File Checks
- All files within `FUSE_SOLID_MAX_LINES`: ✅
- Interfaces location: ✅
- Documentation: ✅
...
□ ZERO linter errors
□ ZERO type errors
□ All files within `FUSE_SOLID_MAX_LINES` verified
□ Build successful
...
```bash
jq --arg p "validation" '.tasks[.current_task].phase = $p' .cursor/apex/task.json \
