...
```text
[ ] Controllers remain focused on HTTP orchestration
[ ] Models remain focused on domain state and relationships
[ ] Services remain focused on one business responsibility
[ ] ALL files stay within `FUSE_SOLID_MAX_LINES` (default `200`)
```
