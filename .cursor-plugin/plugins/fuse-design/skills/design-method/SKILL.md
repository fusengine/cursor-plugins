...
2. **Read `../design-system/SKILL.md`** — always, unconditionally, before anything else
   below. Thin pointer file, cheap to reload. This mandatory direct read is intended by the
   local harness source as the phase-1 trigger, so it runs even on a task needing no new
   token; actual Cursor hook execution and state advancement remain runtime-unverified.
   Existing `design-system.md` tokens still win over its defaults once resolved.
