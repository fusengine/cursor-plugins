...
| Layer | Responsibility | Example | Source-size rule |
|-------|-----------------|---------|------------------|
| **Handler** | HTTP I/O, request parsing, response formatting | Parse JSON, call service, return HTTP response | `FUSE_SOLID_MAX_LINES` (default 200) |
| **Service** | Business logic, orchestration, workflows | Calculate order total, update inventory | `FUSE_SOLID_MAX_LINES` (default 200) |
| **Repository** | Data access, queries, persistence | SELECT, INSERT, UPDATE operations | `FUSE_SOLID_MAX_LINES` (default 200) |
| **Port** | Interface contract | UserRepository, EmailSender interfaces | `FUSE_SOLID_MAX_LINES` (default 200) |
| **Model** | Domain entities, value objects | User struct, Order struct | `FUSE_SOLID_MAX_LINES` (default 200) |
| **Core Service** | Shared business logic (cross-feature) | Email sending, caching, pagination | `FUSE_SOLID_MAX_LINES` (default 200) |
...
1. Genuinely shared across multiple features
...
- [ ] Services use repositories/ports and repositories contain data access only
