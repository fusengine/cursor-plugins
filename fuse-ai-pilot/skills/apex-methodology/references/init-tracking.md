...
Every artefact in `.cursor/apex/` (task id, docs, verify/elicit proofs) is keyed on one `{task-slug}`, derived — in order of preference — from:
...
jq -n --arg slug "$TASK_SLUG" --arg subject "$TASK_SUBJECT" --arg now "$NOW" \
  '{current_task: $slug, created_at: $now, tasks: {($slug): {
    ...
    }}}' > .cursor/apex/task.json
echo "✅ APEX tracking initialized in $(pwd)/.cursor/apex/ (task: $TASK_SLUG)"
```
...
This creates:
- `.cursor/apex/task.json` — tracks documentation consultation status, current phase, and subject (see `phase` field, updated by each phase reference — `references/00-init-branch.md` through `references/09-create-pr.md`)
- `.cursor/apex/docs/` — stores consulted documentation summaries

**Agents must not use Write/Edit until documentation is consulted; PreToolUse hook denial is intended defense-in-depth, and actual Cursor blocking remains runtime-unverified with the unpinned harness.**
