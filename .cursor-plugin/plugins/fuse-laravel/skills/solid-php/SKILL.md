---
name: solid-php
description: Use when applying SOLID principles to Laravel 13 / PHP 8.3+ code — the repository source-size rule, interface placement, or PHPDoc enforcement.
versions:
  laravel: "13.0"
  php: "8.3"
user-invocable: true
...
`FUSE_SOLID_MAX_LINES` (default 200) as the only source-size ceiling, interfaces separated into
...
4. If code is genuinely shared across features → create it in shared Services or Traits
...
|------|---------|----------|
| HTTP handling | Controller | Controllers/ |
| Validation | FormRequest | Requests/ |
| Single operation | Action | Actions/ |
| Complex logic | Service | Services/ |
| Data access | Repository | Repositories/ |
| Data structure | DTO | DTOs/ |
...
- Split controllers, models, and services by responsibility when cohesion drops.
...
|--------------|-----------|-----|
| Source files exceeding `FUSE_SOLID_MAX_LINES` (default 200) | Line count | Split by responsibility |
| Controllers mixing responsibilities | Responsibility review | Extract to Service |
...
| Fat classes | Public API mixes responsibilities | Split responsibilities |
| Duplicated logic | Same logic repeated across files | Extract to Service/Trait |
...
| Split by responsibility before reaching `FUSE_SOLID_MAX_LINES` | Wait until cohesion collapses |
