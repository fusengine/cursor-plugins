...
└── references/                 # Conceptual docs
    ├── installation.md
    ├── core-concepts.md
    └── templates/              # Complete code, split when required
        └── basic-setup.md
```

Every file in this structure must stay within `FUSE_SOLID_MAX_LINES`, a positive integer that
defaults to `200`.
