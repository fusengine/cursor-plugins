...
This is deliberately ask-first, not write-then-diff. Compaction is inherently lossy — it deletes lines — and MEMORY/LESSON.md is committed team memory intended for hook delivery into every session and subagent; current Cursor delivery remains incomplete and runtime-unverified. An unwanted compaction that already overwrote the file is far harder to notice and walk back than one caught in review. Nobody misses a lesson until the mistake it prevented happens again.
