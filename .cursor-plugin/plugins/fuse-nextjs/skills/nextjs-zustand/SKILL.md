...
| File | Purpose |
|------|---------|
| `store.ts` | Store creation with create() |
| `store.interface.ts` | TypeScript interfaces |
| `store-provider.tsx` | Context provider (App Router) |
| `use-store.ts` | Custom hook with selector |
