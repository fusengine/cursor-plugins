...
├── Views/ProfileView.swift        (focused presentation)
├── ViewModels/ProfileViewModel.swift  (focused presentation state)
├── Models/UserProfile.swift       (focused domain model)
├── Protocols/ProfileServiceProtocol.swift
└── Services/ProfileService.swift  (focused service)
```
...
- [ ] Plan file splits if `FUSE_SOLID_MAX_LINES` would be exceeded
