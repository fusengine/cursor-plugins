...
4. **Source-size ceiling** - Keep source files within `FUSE_SOLID_MAX_LINES` (default 200) and split by responsibility (SOLID)
...
**Source-size ceiling**: All source files stay within `FUSE_SOLID_MAX_LINES` (default 200). Split by responsibility when cohesion drops.
...
- Exceed `FUSE_SOLID_MAX_LINES` (positive integer, default `200`); split by responsibility into
  submodules or services before reaching the ceiling
