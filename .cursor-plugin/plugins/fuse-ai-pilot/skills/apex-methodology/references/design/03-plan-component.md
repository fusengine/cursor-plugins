...
```text
Component plan:
- HeroSection.tsx: ~60 lines (within `FUSE_SOLID_MAX_LINES` ✓)
...
```text
If the component approaches `FUSE_SOLID_MAX_LINES` or accumulates multiple responsibilities:
...
[ ] Tasks are atomic
[ ] File sizes estimated within `FUSE_SOLID_MAX_LINES` (default `200`)
