...
when-to-use: "When planning a page's first screen (generate step 4, redesign, any macrostructure pick) — always paired with a body-sequence pick from body-sequence-bank.md; naming one without the other is an incomplete plan."
keywords: hero, first-screen, macrostructure, variety, anti-slop, skeleton
priority: high
related: body-sequence-bank.md, ../SKILL.md, register/brand.md, ../../design-web/references/layout-discipline.md, ../../design-web/references/design-inspiration.md
---
...
`body-sequence-bank.md` holds fifteen body sequences read off shipped code, each with the
...
2. **Editorial masthead** — oversized display type as the dominant hero element, minimal
   or no supporting imagery, content-first, magazine-style column grid below.
3. **Scroll-driven narrative** — a single vertical story with no discrete "hero then
   sections" break; each viewport reveals the next beat (product-tour / storytelling sites).
4. **Sidebar-anchored** — a persistent left/right rail (nav, filters, or a running visual
   motif) with the main content scrolling independently. Common for docs/tool marketing.
5. **Bento-first** — the hero itself IS an asymmetric bento grid (mixed cell sizes, at
   least one large hero cell), not a plain text stack — see bento cell-count rule.
6. **Diagonal/broken-grid** — deliberately off-axis sections (rotated panels, overlapping
   layers, non-rectangular section boundaries) instead of a stacked-rectangle rhythm.
7. **Data-dashboard-as-hero** — for product/SaaS sites, a live or simulated product UI
   takes the hero spot instead of marketing copy, with copy as a caption/overlay.
