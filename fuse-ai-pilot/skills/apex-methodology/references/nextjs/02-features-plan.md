...
| Component Type | Planning rule |
| --- | --- |
| Page (app/) | Keep routing and composition focused |
| Server Component | Keep server rendering/data responsibility focused |
| Client Component | Keep interactive responsibility focused |
| Server Action | Keep one mutation responsibility |
| API Route | Keep one HTTP resource responsibility |

Every file must stay within `FUSE_SOLID_MAX_LINES` (default `200`), the only file-size ceiling.
